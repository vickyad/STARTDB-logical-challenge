...
Array.prototype.customMap = function (callback, context) {
    ...
    const arraySize = this.length
    for (let i = 0; i < arraySize; i++) {
        finalArray.push(callback.call(context, this[i], i, this))
    }
    return finalArray
}

Array.prototype.customReduce = function (callback, initialValue) {
    let finalValue = initialValue ? initialValue : 0
    const arraySize = this.length
    for (let i = 0; i < arraySize; i++) {
        finalValue = callback.call(undefined, finalValue, this[i], i, this)
    }
    return finalValue
}
...
const medalhasPorPais =  olympicsMedalTable.customMap(element => ({country: element.country, medals: element.gold + element.silver + element.bronze}))
console.log(medalhasPorPais)

// 3 - Crie um algoritmo para encontrar os países que conquistaram mais que 10 medalhas de ouro
const paisesCom10MedalhasOuroNoMinimo =  olympicsMedalTable.customFilter(element => element.gold > 10).customMap(element => element.country)
...
const paisesCom30MedalhasNoMinimo =  medalhasPorPais.customFilter(element => element.medals >= 30).customMap(element => element.country)
...
const totalGoldMedalsInSouthAmerica = olympicsMedalTable.customFilter(element => element.continent === "AMERICA DO SUL").customMap(element => element.gold).customReduce((total, quantity) => total + quantity) 
const paisesComPeloMenos20MedalhasDeOuro =  totalGoldMedalsInSouthAmerica >= 20 ? true : false 
